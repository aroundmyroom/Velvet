...
- **Two new accessible themes.** A **High-contrast** theme (black/white text on black at AAA-level contrast, with a yellow accent + focus ring so controls, fills and the active state never blend into white text or borders) and a **Colorblind-safe** theme (a blue/orange palette that never relies on red↔green distinctions) join the theme switcher, which now wraps to fit the five options. Both persist like the other themes.
