...
**Append-only queue management keeps the device's own queue from ever shrinking, so it periodically trims already-played tracks from the front.** Confirmed on hardware: a Sonos device in NORMAL play mode that runs its queue dry reports `CurrentTrack` back to `1` rather than simply stopping in place. Under an append-only design, "track 1" is whatever was cast first — potentially hours earlier — so if the append above ever lags behind (Auto-DJ pick latency, a slow database during a library scan, any timing hiccup) and the device genuinely runs out for a moment, an old, already-played track can become audible instead of what Auto-DJ actually queued next. Once the device is more than a handful of tracks into its own queue, `queue/trim` removes everything before a small trailing history (`RemoveTrackRangeFromQueue`) — playback of the current track is completely undisturbed; only "track 1" moves forward with it. `cast-queue` also explicitly sets play mode to `NORMAL` on every fresh cast, so a repeat/shuffle setting left on from the Sonos app can't cause the same kind of surprise.
