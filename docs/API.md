...
| `POST` | `/api/v1/db/random-songs` | `{ minRating?, ignoreList?, ignorePercentage?, ignoreVPaths?, genres?, genreMode?, returnAll? }` | One random track. `ignoreList` lets callers avoid repeats across calls. `genres` is an array of display genre names (from `/api/v1/db/genres`); `genreMode` is `"whitelist"` (default) or `"blacklist"`. `returnAll: true` switches to Auto-DJ batch mode: returns a broad candidate pool (scoped to collections/rating/artists/cooldown only, no BPM/key/genre filtering) for client-side soft scoring instead of one server-picked track — see `docs/autodj-scoring.md`. |
