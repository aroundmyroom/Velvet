...
export function getUiAddress() {
  if (typeof uiAddress !== 'string') { throw new TypeError('Syncthing UI Address Not Set'); }
  return uiAddress;
}
...
// NOTE(pending): change this for server reboot
export async function setup() {
  if (config.program.federation.enabled === false) { return kill2(); }

  // Guard: bail early if the syncthing binary doesn't exist on this platform.
  const binaryName = osMap[platform];
  if (!binaryName) {
    winston.error(`Syncthing: unsupported platform '${platform}' — federation disabled.`);
    return;
  }
  const binaryPath = path.join(__dirname, `../../bin/syncthing/${binaryName}`);
  if (!fs.existsSync(binaryPath)) {
    winston.error(`Syncthing binary not found at ${binaryPath} — federation disabled. Disable federation in admin to suppress this message.`);
    return;
  }
...
function initSyncthingConfig() {
  return new Promise((resolve, reject) => {
    ...
    newProcess.stdout.on('data', (data) => {
      winston.info(`SYNCTHING: ${String(data).trim()}`);
    });

    newProcess.stderr.on('data', (data) => {
      winston.info(`SYNCTHING ERROR: ${String(data).trim()}`);
    });

    newProcess.on('close', (code) => {
      if (code !== 0) {
        winston.error('Syncthing: Failed to setup new directory');
        return reject(new Error('Syncthing init failed'));
      }
      resolve();
    });
  });
...
function getSyncthingId() {
  return new Promise((resolve, reject) => {
    ...
    newProcess.on('close', (code) => {
      if (code !== 0) {
        winston.error('SyncThing: Failed to setup new directory');
        return reject(new Error('Get Syncthing ID failed'));
      }
      resolve();
    });
  });
...
function loadConfig() {
  ...
  if (typeof xmlObj.configuration.folder === 'object' && !Array.isArray(xmlObj.configuration.folder)) {
  ...
  // convert objects to arrays
  if (typeof xmlObj.configuration.device === 'object' && !Array.isArray(xmlObj.configuration.device)) {
...
export function addDevice(deviceId, directories) {
  ...
  xmlObj.configuration.folder.forEach(f => {
    ...
    if (directories[f['@_label']]) {
      // Modify devices
      if (typeof f.device === 'object' && !Array.isArray(f.device)) {
        f.device = [f.device];
  ...
  try {
    ...
    spawnedProcess.on('close', (_code) => {
      ...
      if (bootFailCount >= MAX_BOOT_FAILS) {
        winston.error(`Syncthing crashed ${bootFailCount} times in a row — giving up. Disable federation in admin to clear this state.`);
        bootFailCount = 0;
        spawnedProcess = undefined;
        return;
      }
      winston.info(`Syncthing failed (attempt ${bootFailCount}/${MAX_BOOT_FAILS}). Retrying in 4s…`);
      setTimeout(() => {
        spawnedProcess = undefined;
        bootProgram();
      }, 4000);
    ...
    spawnedProcess.on('spawn', () => { bootFailCount = 0; });
