...
| `GET` | `/api/v1/sonos/transport-status?ip=` | — | Poll playback state: `{ playing, paused, stopped, state, position, duration, track, nrTracks, trackUri, trackFp, trackTitle, trackArtist, trackAlbum, trackArt }` (`track` = current queue position, `nrTracks` = device queue length; together they map the device queue onto the player queue. `trackFp` is the `vpath/filepath` recovered from the stream URI — exact track identity, used instead of title/artist matching to tell a natural advance from a Sonos-app takeover. `trackFp` is `null` for non-Velvet content. The `track*` metadata fields drive now-playing for external Sonos-Favourite content.). |
...
| `POST` | `/api/v1/sonos/queue/append` | `{ ip, tracks[] }` | Append upcoming tracks to the end of the Sonos queue **without wiping it** — playback, queue history and track number are untouched. Used to slide the mirrored window while Sonos advances on its own. |
| `POST` | `/api/v1/sonos/queue/trim` | `{ ip, currentTrack, keep? }` | Remove already-played tracks from the front of the device's queue (`RemoveTrackRangeFromQueue`), keeping `keep` (default 2) for history. Playback is undisturbed. Keeps "track 1" from staying stale indefinitely under append-only queue management — a device that runs its queue dry reports `CurrentTrack` back to 1 rather than just stopping, so an ancient track 1 becomes audible if the topup above ever lags. |
| `GET` | `/api/v1/sonos/events/stream` | — | **Server-sent events.** Transport changes the speaker pushes to Velvet (UPnP GENA) are forwarded as `data:` frames `{ ip, state, playing, paused, stopped, track, nrTracks, trackUri, trackFp }`. Lets the player react to a track change immediately instead of on the next poll. Polling continues as the fallback — this stream stays empty where the speaker cannot reach Velvet's callback. |
...
| `POST` | `/api/v1/sonos/queue/jump` | `{ ip, track }` | Jump to a track already in the Sonos queue by 1-based position (`Seek TRACK_NR`). One call, and the queue survives — used for skips instead of rebuilding. `409` if `track` is beyond the queue length. |
