...
function appendLivePlaylistOption(name) {
  const select = document.getElementById('live-playlist-select');
  if (!select) return;
...
function renderAlbum(id, artist, name, albumArtFile, year) {
  return `<li class="collection-item">
    <div ${year ? `data-year="${escapeHtml(year)}"` : '' } ${artist ? `data-artist="${escapeHtml(artist)}"` : '' } ${id ? `data-album="${escapeHtml(id)}"` : '' } class="albumz flex2" onclick="getAlbumsOnClick(this);">
        ${albumArtFile ? 
          `<img class="album-art-box" loading="lazy" src="${MSTREAMAPI.currentServer.host}album-art/${encodeURIComponent(albumArtFile)}?compress=s&token=${encodeURIComponent(MSTREAMAPI.currentServer.token)}">`: 
        ...
        <span><b>${escapeHtml(name)}</b> ${year ? `<br>[${escapeHtml(year)}]` : ''}</span>
    </div>
  </li>`;
}
...
function renderFileWithMetadataHtml(filepath, id, metadata) {
  return `<li data-id="${id}" class="collection-item">
    <div data-file_location="${escapeHtml(filepath)}" class="filez flex" onclick="onFileClick(this);">
      <img class="album-art-box" loading="lazy" ${metadata['album-art'] ? `src="${MSTREAMAPI.currentServer.host}album-art/${encodeURIComponent(metadata['album-art'])}?compress=s&token=${encodeURIComponent(MSTREAMAPI.currentServer.token)}"` : 'src="assets/img/default.png"'}>
      <div>
        <b><span>${(!metadata || !metadata.title) ? escapeHtml(filepath.split("/").pop()) : `${escapeHtml(metadata.title)}`}</span></b>
  ...
  return `<li class="collection-item">
    <div data-file_location="${escapeHtml(fileLocation)}" class="filez ${aa ? 'flex2' : ''}" onclick="onFileClick(this);">
      ...
      <span>
        ...
        <span class="${aa ? '' : 'item-text'}">${rating ? `[${escapeHtml(rating)}] ` : ''}${escapeHtml(title)}</span>
...
async function senddir(root) {
  ...
  try {
    ...
    document.getElementById('directoryName').textContent = response.path;
...
async function init() {
  try {
    ...
    response.playlists.forEach(p => {
      ...
      appendLivePlaylistOption(p.name);
    });
...
async function getAllPlaylists() {
  ...
  try {
    ...
    setSafeHtmlById('filelist', playlists);
  }catch (err) {
...
async function newPlaylist() {
  ...
  try {
    ...
    appendLivePlaylistOption(title);
...
function getMobilePanel(){
  ...
  document.getElementById('filelist').innerHTML = 
    `<div class="mobile-links pad-6">
      <a target="_blank" href="https://play.google.com/store/apps/details?id=velvet.music&pcampaignid=MKT-Other-global-all-co-prtnr-py-PartBadge-Mar2515-1">
        <img alt='Get it on Google Play' src='https://play.google.com/intl/en_us/badges/images/generic/en_badge_web_generic.png'/>
      </a>
    </div>
    <div class="mobile-links pad-6">
      <a target="_blank" href="https://apps.apple.com/us/app/velvet-player/id1605378892">
  ...
  setSafeHtmlById('filelist', 
    ...
    <div class="flex">
      <label class="grow" for="search-in-artists">
        <input id="search-in-artists" class="filled-in" type="checkbox">
        <span>Artists</span>
      </label>
      <label class="grow" for="search-in-albums">
        <input id="search-in-albums" class="filled-in" type="checkbox">
        <span>Albums</span>
      </label>
      <label class="grow" for="search-in-titles">
        <input id="search-in-titles" class="filled-in" type="checkbox">
        <span>Song Titles</span>
      </label>
      <label class="grow" for="search-in-filepaths">
        <input id="search-in-filepaths" class="filled-in" type="checkbox">
