const VUEPLAYERCORE = (() => {
  ...
  velvetModule.livePlaylist = {
    name: false
  };

  velvetModule.altLayout = {
    'moveMeta': false,
  ...
  try {
    ...
    velvetModule.altLayout.flipPlayer = typeof altLayout.flipPlayer === 'boolean' ? altLayout.flipPlayer : false;
    velvetModule.altLayout.audioBookCtrls = typeof altLayout.audioBookCtrls === 'boolean' ? altLayout.audioBookCtrls : false;
    velvetModule.altLayout.moveMeta = typeof altLayout.moveMeta === 'boolean' ? altLayout.moveMeta : false;
  ...
  new Vue({
    ...
    data: {
      ...
      playlists: velvetModule.playlists,
      showClear: showClearLink,
      altLayout: velvetModule.altLayout,
      meta: MSTREAMPLAYER.playerStats.metadata,
      livePlaylist: velvetModule.livePlaylist
    },
    computed: {
    ...
    methods: {
      ...
      checkMove: function (event) {
        ...
        if (velvetModule.livePlaylist.name) {
          const songs = [];
          for (let i = 0; i < MSTREAMPLAYER.playlist.length; i++) {
            songs.push(MSTREAMPLAYER.playlist[i].filepath);
          }
          MSTREAMAPI.savePlaylist(velvetModule.livePlaylist.name,songs, true);
        }
      },
  ...
  velvetModule.addSongWizard = async (filepath, metadata, lookupMetadata, position, livePlaylist, autoPlayOff) => {
    // Escape filepath
    ...
    if (position) {
      ...
      if (velvetModule.livePlaylist.name && livePlaylist !== false) {
        await MSTREAMAPI.addToPlaylist(velvetModule.livePlaylist.name, newSong.filepath);
      }
    }
