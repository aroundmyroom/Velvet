...
| `GET` | `/api/v1/sonos/transport-status?ip=` | — | Poll playback state: `{ playing, paused, stopped, state, position, duration, track, trackUri, trackTitle, trackArtist, trackAlbum, trackArt }` (`track` = current queue position; the player uses it to detect Sonos-app navigation and cede control. The `track*` metadata fields drive now-playing for external Sonos-Favourite content.). |
